...
# To update, run: docker buildx imagetools inspect node:22-bookworm (or podman)
...
RUN --mount=type=cache,id=openclaw-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    ...
    DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
      procps hostname curl git openssl gh
...
COPY --from=runtime-assets --chown=node:node /app/scripts/docker-start-gateway.sh /app/scripts/docker-start-gateway.sh
COPY --from=runtime-assets --chown=node:node /app/scripts/docker-healthcheck.sh /app/scripts/docker-healthcheck.sh
